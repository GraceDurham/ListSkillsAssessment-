...
def halvesies(numbers):
    ...
    list_of_half_numbers = []

    for num in numbers:
        number_divided_two = float(num)/2
        list_of_half_numbers.append(number_divided_two)

    
    return list_of_half_numbers
...
def word_lengths(words):
    ...
    for word in words:
        new_word = len(word)
        new_word_lengths.append(new_word)

    return new_word_lengths
...
def mult_numbers(numbers):
    ...
    total = 1  #start count at 1 because numbers multiplied by zero is zero

    for num in range(0, len(numbers)): #use for loop start at index zero use len to stop and the end of the index
        total = total * numbers[num] #increment use total multiplied by the numbers at that index
...
def join_strings(words):
    ...
    for the_string in words:
        joined_strings = joined_strings + the_string
...
def average(numbers):
    ...
    """
    #Best way to handle an empty list is with try and except code is below I had to 
    #fix the indentation of line 239 and line 241 
 #   try: 
 #       numbers != []
...
#    print "Please input a number in the empty list can not divide by zero"
...
def join_strings_with_comma(words):
    ...
    joined_strings_with_comma = joined_strings_with_comma.join(words)
...
#given a word and a letter return the index of that letter
#example find_index('dog', 'o') would return 1
...
#given an array of words and a letter 
#return an array that has the first index of that letter in each word
#example find_letter_indices(['odd', 'dog', 'who', 'jumps'], 'o')  returns [0,1,2,None]
def find_letter_indices(words, letter):
